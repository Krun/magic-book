...
Obtain k equally sized and
random subsets of the training data. For each of these k subsets, build a model
using the remaining k −1 sets and evaluate this model on the k th subset. Store
the performance of the model and repeat this process for all remaining subsets.
In the end, we have k performance measures, all obtained by testing a model
on data not used for its construction, and that is the key issue. The k -fold
cross-validation estimate is the average of these k measures. A common choice
for k is 10. Sometimes we even repeat the overall k-fold CV process several
times to get even more reliable estimates.
In general, we can say that when facing a predictive task, we have to make
the following decisions:
• Select the alternative models to consider (the models can actually be
alternative settings of the same algorithm) for the predictive task(s) we
want to address.
